if (typeof SlimStatParams == 'undefined') SlimStatParams = {'extensions_to_track': ''};
var SlimStat = {
	// Private Properties
	_id : '-1.0',
...
if (typeof SlimStatParams.disable_outbound_tracking == 'undefined'){
	...
	var type = 0; // Outbound Link
	var skip_this_link = false;

	for (var i=0; i<links_in_this_page.length; i++) {
		if (extensions_to_track.length > 0){
			extension_current_link = links_in_this_page[i].pathname.split('.').pop().replace(/[\/\-]/g, '');
			skip_this_link = !SlimStat.in_array(extension_current_link, extensions_to_track);
			type = 1; // Download
		}

		if (skip_this_link && (links_in_this_page[i].hostname == location.hostname || links_in_this_page[i].href.indexOf('://') == -1 || links_in_this_page[i].className.indexOf('noslimstat') != -1)){
			continue;
		}

		if (links_in_this_page[i].addEventListener) {
			links_in_this_page[i].addEventListener("click", function (i, type) { return function (e) { SlimStat.ss_track(e, type, "A:"+(i+1)) } }(i, type), false);
		}
		else if (links_in_this_page[i].attachEvent){
			links_in_this_page[i].attachEvent("onclick", function (i, type) { return function (e) { SlimStat.ss_track(e, type, "A:"+(i+1)) } }(i, type));
		}
	}
}
...
if (current_data.length	) SlimStat.send_to_server(current_data+"&sw="+(screen.width||window.innerWidth||document.documentElement.clientWidth||document.body.offsetWidth)+"&sh="+(screen.height||window.innerHeight||document.documentElement.clientHeight||document.body.offsetHeight)+"&cd="+screen.colorDepth+"&aa="+SlimStat.has_smoothing()+"&pl="+SlimStat.detect_plugins());